...
Java_com_example_smartgallery_NativeBridge_nativeDHash(JNIEnv *env, jobject /* this */,
    ...
    for (int y = 0; y < dhashH; ++y) {
        for (int x = 0; x < dhashW - 1; ++x) {
            ...
            if (left > right) hash |= (1ULL << bitIndex);
            bitIndex++;
        }
    }
...
// NEW: return raw laplacian variance as double (for calibration / plotting)
extern "C" JNIEXPORT jdouble JNICALL
Java_com_example_smartgallery_NativeBridge_nativeLapVariance(JNIEnv *env, jobject /* this */,
                                                     jintArray jpixels, jint width, jint height) {
    if (jpixels == nullptr) return 0.0;
    jint *pixels = env->GetIntArrayElements(jpixels, nullptr);
    if (pixels == nullptr) return 0.0;
